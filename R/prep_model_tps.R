...
#'   prepped_points = tpts,
#'   covars = reduced_vars, out_dir = out_dir, outname = "model_input_pts.rds"
#' )
#' }
prep_model_tps <- function(
  ...
  bgc_pts_subzone <- lapply(zones, function(i) {
    ...
    # TODO: check this non-conforming calls. Also this is based on structure fo ESST
    ...
    tdat <- tdat |>
      dplyr::select(
        ...
        .data$tid, .data$slice, .data$bgc_cat, .data$X, .data$Y,
