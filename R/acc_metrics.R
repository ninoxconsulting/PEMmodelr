...
acc_metrics <- function(pred_data, fuzz_matrix, theta = NULL) {
  ...
  aspat_pred2 <- aspat_pdata |>
    dplyr::mutate_if(is.factor, as.character) |>
    dplyr::mutate(.pred_class = ifelse(.data$.pred_class == .data$mapunit2, as.character(.data$mapunit1), as.character(.data$.pred_class))) |>
  ...
  aspatial_pa_acc <- dplyr::left_join(aspatial_acc, aspat_pred2, by = c("mapunit1" = ".pred_class")) |>
    ...
    dplyr::ungroup() |>
    dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~ tidyr::replace_na(., 0)))# |>
   # dplyr::mutate(aspat_pa_theta1 = mean(aspat_pa, na.rm = TRUE),
  #                aspat_pa_theta0 = sum(aspat_pa_correct/sum(trans.tot)))
  ...
  # note if there are not trans.tot for a class - we can change this to 0 as creating NAN values
